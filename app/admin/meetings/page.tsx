  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(payload?.error || "Gagal menyimpan laporan.");
      }
    } catch {
      ...
      <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden min-w-0">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={goToday}
              className="text-xs font-medium px-2 py-0.5 rounded bg-blue-100 text-blue-700 hover:bg-blue-200 transition-colors"
            >
              Hari Ini
            </button>
          </div>
                ...
                <div className="space-y-0.5">
                  {dayMeetings.slice(0, 3).map((m) => (
                    <div
                      ...
                      title={m.title}
                    >
                      {new Date(m.meeting_date).toLocaleTimeString("id-ID", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}{" "}
                      {m.title}
          ...
          <div className="px-5 py-4 border-b border-slate-100 bg-slate-50">
            ...
            {selectedDay && (
              <p className="text-xs text-slate-500 mt-0.5">
                {selectedMeetings.length} jadwal
              </p>
            )}
          </div>
          ...
          <div className="space-y-3">
            {[...meets]
              ...
              .map((meet) => (
                <div
                  key={meet.id}
                  className="text-sm p-3 bg-slate-50 rounded-md border border-slate-100 flex flex-col gap-1"
                >
                  <div className="flex justify-between items-start">
                    <strong className="text-slate-800">{meet.title}</strong>
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded font-medium ${meet.completion_status === "terlewat" ? "bg-red-100 text-red-700 border border-red-200" : "bg-green-100 text-green-700 border border-green-200"}`}
                    >
                      {meet.completion_status === "terlewat"
                        ? "Terlewat"
                        : "Selesai"}
                    </span>
                  </div>
...
export default function MeetingsPage() {
  ...
  const [reportingMeeting, setReportingMeeting] = useState<Meeting | null>(
    null,
  );
  ...
  const handleDelete = async (meeting: Meeting) => {
    ...
    if (isSeries) {
      const res = confirm(
        "Jadwal ini berulang. Hapus sesi ini saja (OK), atau hapus SESI INI DAN SETERUSNYA (Cancel)?",
      );
      if (!res) {
        const res2 = confirm(
          "Anda yakin ingin menghapus SESI INI DAN SETERUSNYA?",
        );
        if (!res2) return;
        mode = "series";
      }
    ...
    try {
      ...
      if (mode === "series") {
        ...
        url.searchParams.set(
          "sessionNumber",
          meeting.session_number.toString(),
        );
      }
      ...
      if (!res.ok) {
        throw new Error(payload?.error || "Gagal menghapus jadwal.");
      }

      await fetchMeetings();
      setNotice({ type: "success", text: "Jadwal berhasil dihapus." });
    } catch (error) {
      console.error(error);
      setNotice({
        type: "error",
  ...
  const handleExportCSV = () => {
    ...
    const rows = meetings.map((meet) => {
      ...
      const status = meet.is_completed
        ? "Selesai"
        : meetDate > now
          ? "Mendatang"
          : "Belum Dilaporkan";
      const sesiStr =
        meet.session_count > 1
          ? `${meet.session_number}/${meet.session_count}`
          : "1/1";
      return [
        `"${meet.title}"`,
        `"${sesiStr}"`,
    ...
    link.setAttribute(
      "download",
      `Data_Jadwal_LMS_${new Date().toISOString().slice(0, 10)}.csv`,
    );
  ...
  const pastMeetings = meetings.filter(
    (m) =>
      m.is_completed ||
      new Date(m.meeting_date).getTime() + 60 * 60 * 1000 <= now.getTime(),
  );
      ...
      {editingMeeting && (
        <EditMeetingModal
          ...
          onSuccess={() => {
            ...
            fetchMeetings();
            setNotice({ type: "success", text: "Jadwal berhasil diperbarui." });
          }}
        />
      )}
      {reportingMeeting && (
        <ProgressReportModal
          ...
          onSuccess={() => {
            ...
            setNotice({
              type: "success",
              text: "Laporan progress berhasil disimpan.",
            });
          }}
        />
      )}
