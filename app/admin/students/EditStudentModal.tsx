...
export default function EditStudentModal({ student, onClose, onSuccess }: EditStudentModalProps) {
  ...
  const [contact, setContact] = useState(student.email_or_phone || "");
  const [mpin, setMpin] = useState(student.mpin || "");
  const [grade, setGrade] = useState(student.grade ? student.grade.replace("Kelas ", "") : "1");
  const [bio, setBio] = useState(student.bio || "");
  
  const [availableModules, setAvailableModules] = useState<Module[]>([]);
  const [selectedModules, setSelectedModules] = useState<number[]>(student.modules.map(m => m.id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Gagal mengedit siswa.");
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg rounded-xl bg-white shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <h2 className="text-lg font-semibold text-slate-900">Edit Siswa</h2>
